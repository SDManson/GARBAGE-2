...
string StationHomeName = "8-KZXQ - FORT Alpine";
//var MaxInLocal = 0;
Sanderling.Parse.IMemoryMeasurement Measurement => Sanderling?.MemoryMeasurementParsed?.Value;

string ShipName = Measurement?.WindowInventory?[0]?.LeftTreeListEntry?[1]?.Text;
//.Remove(Measurement?.WindowInventory?[0]?.LeftTreeListEntry?[1]?.Text.LastIndexOf("("));
ShipName=ShipName.Remove(ShipName.LastIndexOf("("));
Sanderling.InvalidateMeasurement();
Sanderling.WaitForMeasurement();

WindowChatChannel chatLocal =>Measurement?.WindowChatChannel.FirstOrDefault(x=> x.Caption.Contains("Local")); 
//WindowChatChannel chatLocal =>Sanderling?.MemoryMeasurementParsed?.Value.WindowChatChannel.FirstOrDefault(); 
//Sanderling.MemoryMeasurementParsed?.Value?.WindowChatChannel.FirstOrDefault();
	Host.Log(chatLocal.Caption);
...
var MaxInLocal = EntrylocalCount + 70; //change at you needs (+100 if you want)
...
string CharName =firstname.Remove(firstname.IndexOf(" "));// Your char name. The bot will create a filename with this name, into executable sanderling folder
 Host.Log( CharName);
 //return;
...
var	MenuEntries =
    ...
    ?.ToArray();
Sanderling.MouseMove(MenuEntries?.FirstOrDefault(x => x.Text.Contains("Stargates")));
...
IntelFor.Add("Geminate", "Bean-Intel");
...
string SecondAnomalyToTake = "Forsaken Hub"; // his name , ex:  "forsaken hub" or " combat" Blood Raider Rally Point
...
int TargetCountMax = 4; //target numbers; ex: 4
...
string [] PermanentActive = new [] {"Omnidirectional", "Sensor", "Auto", "rash"};
///////    RATTING SHIP //////////////
...
//----------- Studleys Personalized Keys
var launchDrones = VirtualKeyCode.VK_L;
var recallDrones  = VirtualKeyCode.VK_R;
var probeKey= new [] {VirtualKeyCode.LMENU,VirtualKeyCode.VK_P};
//var bookKey= new [] {VirtualKeyCode.RCONTROL, VirtualKeyCode.VK_B};
var   bookKey = VirtualKeyCode.CAPITAL;
/////
...
Host.Log("UTC start at:  " +dateAndTime.ToString(new System.Globalization.CultureInfo("en-US"))+ " .");
...
Host.Log(" >  eveRealServerDT :  " +eveRealServerDT.ToString(new System.Globalization.CultureInfo("en-US"))+ " .");
}
var eveSafeDT = eveRealServerDT.AddHours(-hoursToDT).AddMinutes(-minutesToDT);
Host.Log(" >  eveSafeDT : " +eveSafeDT.ToString(new System.Globalization.CultureInfo("en-US"))+ " .");
 //just some calcs
...
for(;;)
{
var tt= new CultureInfo("en-US",false).TextInfo;
MemoryUpdate();
Host.Log(
		" >   " +tt.ToTitleCase(CharName)+  "    Started  in " +currentLocationName+ " at: " +  startSession.ToString(" HH:mm") +// alternative (" dd/MM/yyyy HH:mm") 
    ...
    if (null !=RetreatReasonDread)
        {
            ...
            if (probeScannerWindow == null)
                Sanderling.KeyboardPressCombined(probeKey);
...
Host.Log("                void  Launching my Vipers");
	Sanderling.KeyboardPress(launchDrones);
	return;
}
...
void DroneReturnToBay()
{
    Host.Log("               I do not forget my Vipers here");
     Sanderling.KeyboardPress(recallDrones);//if you like 
}
...
bool MeasurementEmergencyWarpOutEnter =>    !(Measurement?.IsDocked ?? false) && (!(EmergencyWarpOutHitpointPercent < ArmorHpPercent));
bool ReasonBumped => (0 < ListRatOverviewEntry?.Length) && (listOverviewEntryFriends?.Length > 0) && (listOverviewEntryFriends?.FirstOrDefault()?.DistanceMax < 50);
bool ReasonCapsuled =>(reasonCapsule);
bool ReasonTimeElapsed =>(logoutme);
bool ReasonDrones =>(reasonDrones);
bool ReasonCargoFull =>(OreHoldFilledForOffload || FullCargoMessage);
bool ReasonEnd=>(SiteFinished);
...
void Orbitkeyboard()
{
if (0 == ListCelestialObjects?.Length) OrbitRats();
if (1 == ListCelestialObjects?.Length) {   Sanderling.KeyDown(orbitKeyCode);Sanderling.MouseClickLeft(ListCelestialObjects?.FirstOrDefault()); Sanderling.KeyUp(orbitKeyCode);}
...
else
{
    var SaveLocationWindow = Measurement?.WindowOther?.FirstOrDefault(w =>
                            (w?.Caption.RegexMatchSuccessIgnoreCase("New Location") ?? false));
    Sanderling.KeyboardPress(bookKey);
        Host.Delay(1111);
